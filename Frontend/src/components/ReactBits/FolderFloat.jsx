...
const matterInstance = Matter.Bodies ? Matter : (Matter.default || Matter);
const { Bodies, Body, Composite, Engine } = matterInstance;
...
export default function FolderFloat({
  ...
  const n = list.length;
  const sub = sublabel || `${n} ${n === 1 ? 'capability' : 'capabilities'}`;
  ...
  const startPhysics = useCallback(() => {
    ...
    const zone = {
      ...
      top: Math.min(...ys) - ZONE_PAD - 20,
      bottom: -lift + Math.max(...w.sizes.map(s => s.h)) + 10
    };
    w.zone = zone;
    ...
    >
      ...
      <div className="folder-float__folder">
        ...
        <span className="folder-float__front" aria-hidden="true">
          <div className="folder-float__header-row">
            <span className="folder-float__logo-badge">
              <svg viewBox="0 0 28 28" width="14" height="14" fill="#8B5CF6">
                <rect x="3" y="3" width="16" height="16" rx="4" opacity="0.5" />
                <rect x="9" y="9" width="16" height="16" rx="4" />
              </svg>
            </span>
            <span className="folder-float__hint-pill">Interactive</span>
          </div>
